...
export class Kernel {
    ...
    buildNestContext(): string {
        ...
        lines.push(
            ...
            ``,
            `### Docs`,
            `Read these before writing or modifying plugins:`,
            `- \`${docsDir}/plugins.md\` — plugin structure, NestAPI reference, interfaces, examples`,
            `- \`${docsDir}/architecture.md\` — sessions, message routing, broadcast`,
            `- \`${docsDir}/blocks.md\` — block protocol, interactive prompts`,
            `- \`${docsDir}/config.md\` — config reference`,
            `- \`${typesPath}\` — type definitions`,
        );
